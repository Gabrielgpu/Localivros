...
class WebScraping:
  ...
  async def parse_html(self):
    try:
      ...
      self.book_inf["Descrição Curta"].append(self.get_text('#bookDescription_feature_div .a-expander-partial-collapse-content'))
      ...
      self.book_inf["anoDePublicacao"].append(self.get_text('#rpi-attribute-book_details-publication_date .rpi-attribute-value span'))
      self.book_inf["isbn10"].append(self.get_text('#rpi-attribute-book_details-isbn10 .rpi-attribute-value span'))
      self.book_inf["GTIN/EAN"].append(self.get_text('#rpi-attribute-book_details-isbn13 .rpi-attribute-value span').replace("-", ""))
      self.book_inf["GTIN/EAN da embalagem"].append(self.get_text('#rpi-attribute-book_details-isbn13 .rpi-attribute-value span').replace("-", ""))
      self.book_inf["autor"].append(self.get_text('#bylineInfo .a-link-normal'))
      self.book_inf["Categoria do produto"].append(self.get_text('li:nth-child(5) .a-color-tertiary'))
      ...
      self.book_inf["Preço"].append("".join(preco_completo))
      self.book_inf["Preço novo"].append(self.get_text('.aok-offscreen').split(" ")[0][-5:])
      ...
      self.book_inf["Largura do Produto"].append(width)
      self.book_inf["Altura do Produto"].append(height)
      self.book_inf["Profundidade do produto"].append(depth)
    except Exception as e:
  ...
  async def search_image(self):
    try:
      ...
      self.book_inf["URL Imagens Externas"].append(image)
    except Exception as e:
  ...
  async def search_edition(self):
    try:
      ...
      if edition:
          ...
          edition = ''
      self.book_inf["Informações adicionais"].append(edition)
    except Exception as e:
  ...
  async def display(self):
    print({
      ...
      "Descrição": self.book_inf["Descrição Curta"],
      "Autor": self.book_inf["autor"],
      "Imagem": self.book_inf["URL Imagens Externas"],
      ...
      "ISBN13": self.book_inf["GTIN/EAN"],
      "Altura": self.book_inf["Altura do Produto"],
      "Largura": self.book_inf["Largura do Produto"],
      "Profundidade": self.book_inf["Profundidade do produto"],
      "Edição": self.book_inf['Informações adicionais'],
      "Preço": self.book_inf['Preço'],
      "Preço Novo": self.book_inf['Preço novo'],
      "Categoria": self.book_inf["Categoria do produto"]
    })
  
  async def result(self):
    return {
      "isbn": self.book_inf["GTIN/EAN"][0], 
      "title": self.book_inf["Descrição"][0], 
      "authors": self.book_inf["autor"][0], 
      "description": self.book_inf["Descrição Curta"][0], 
      "image": self.book_inf["URL Imagens Externas"][0], 
      "price": self.book_inf["Preço"][0], 
      "sku": "", 
      "paginas": self.book_inf["numeroDePaginas"][0], 
      "editora": self.book_inf["Marca"][0], 
      "data de Publicação": self.book_inf["anoDePublicacao"][0], 
      "altura": self.book_inf["Altura do Produto"][0], 
      "largura": self.book_inf["Largura do Produto"][0], 
      "profundidade": self.book_inf["Profundidade do produto"][0], 
      "edição": self.book_inf['Informações adicionais'][0],
      "Preço Novo": self.book_inf['Preço novo'][0],
      "Categoria": self.book_inf["Categoria do produto"][0]
    }
